<?php
// MR SAQIBHosting - Professional Web Hosting Platform
session_start();
...
// Every visitor must be logged in to use the hosting panel
authRequireLogin();
$current_username = authCurrentUser();
$USER_DIR = userSitesDir($current_username);

// Is a GitHub account connected in this session?
$github_connected = isset($_SESSION['github_token']) && !empty($_SESSION['github_token']);
$github_username  = $_SESSION['github_username'] ?? null;
        ...
        if ($file['error'] === UPLOAD_ERR_OK) {
            ...
            if (in_array($file_ext, $allowed_types)) {
                ...
                if ($file_ext === 'zip') {
                    ...
                    $result = handleZipUpload($file_tmp, $custom_url, $USER_DIR);
                    if ($result['success']) {
                        $website_url = getWebsiteUrl($custom_url, $current_username);
...
function handleZipUpload($zip_tmp, $custom_url, $user_dir) {
    $extract_path = $user_dir . '/' . $custom_url;
    ...
    if (class_exists('ZipArchive')) {
        ...
        $entries = simpleZipExtractAll($zip_tmp);
        ...
        foreach ($entries as $e) {
            ...
            $file_ext = strtolower(pathinfo($e['name'], PATHINFO_EXTENSION));
            if (!in_array($file_ext, $allowed_extensions)) continue;

            $safe_filename = basename($e['name']);
            file_put_contents($extract_path . '/' . $safe_filename, $e['data']);
            $extracted_files[] = $safe_filename;
...
function getWebsiteUrl($path, $username) {
    $protocol = isset($_SERVER['HTTPS']) && $_SERVER['HTTPS'] !== 'off' ? 'https' : 'http';
    $host = $_SERVER['HTTP_HOST'];
    $base_url = $protocol . '://' . $host . dirname($_SERVER['PHP_SELF']);
    return rtrim($base_url, '/') . '/view.php?user=' . urlencode($username) . '&site=' . urlencode($path);
}

// Get list of uploaded sites (only this logged-in user's own sites)
$uploaded_sites = [];
$total_size = 0;
$github_deployments = ghGetDeployments();
...
if (is_dir($USER_DIR)) {
    ...
    foreach ($files as $file) {
        if ($file !== '.' && $file !== '..') {
            ...
            if (is_dir($file_path)) {
                // Handle directory (ZIP extracted sites, or GitHub deployments)
                $html_files = glob($file_path . '/*.{html,htm}', GLOB_BRACE);
                if (!empty($html_files)) {
                    $main_file = basename(pickMainHtmlFile($html_files));
                    $site_name = $file;

                    $deploy_key = $current_username . '/' . $site_name;
                    $gh_deployment = isset($github_deployments[$deploy_key]) ? $github_deployments[$deploy_key] : null;

                    $uploaded_sites[] = [
                        'name' => $site_name,
                        'filename' => $file,
                        'url' => getWebsiteUrl($site_name, $current_username),
                        'time' => filemtime($file_path),
                        'size' => getDirectorySize($file_path),
                        'type' => $gh_deployment ? 'github' : 'zip',
                        'is_dir' => true,
                        'repo' => $gh_deployment ? $gh_deployment['repo_full_name'] : null,
                        'branch' => $gh_deployment ? $gh_deployment['branch'] : null,
                    ];
...
<head>
    ...
    <title>MR WASEEM HACKER CLOUD | Premium Hosting</title>
    <!-- Fonts -->
    ...
    <style>
        ...
        :root {
            ...
            --shadow-lg: 0 20px 40px rgba(255, 107, 53, 0.2);
            --shadow-md: 0 10px 20px rgba(255, 107, 53, 0.15);
            --shadow-sm: 0 4px 12px rgba(255, 107, 53, 0.1);
            
            --glow-primary: 0 0 15px rgba(255, 107, 53, 0.5);
            --glow-secondary: 0 0 25px rgba(255, 167, 38, 0.3);
        ...
        * { 
        ...
        /* Animated network background (same effect/colors as the login page) */
        #network-bg-canvas {
            position: fixed;
            inset: 0;
            width: 100%;
        ...
        .bg-gradient {
            ...
            background: 
                ...
                radial-gradient(circle at 80% 20%, rgba(255, 167, 38, 0.15) 0%, transparent 50%),
                radial-gradient(circle at 50% 50%, rgba(255, 152, 0, 0.08) 0%, transparent 70%),
                radial-gradient(ellipse at top, #0d1420cc 0%, #05070ccc 70%);
        }
        ...
        /* Owner brand avatar above the heading — outer ring spins, photo stays still */
        .top-avatar-wrap {
            display: flex;
            justify-content: center;
            padding-top: 10px;
        ...
        .top-avatar-ring::after {
            ...
            border: 2px solid transparent;
            border-bottom-color: var(--accent);
            border-left-color: rgba(56, 224, 209, 0.35);
        ...
        .top-avatar-ring img {
            ...
            /* intentionally no animation here — this circle stays still */
        }
...
<body>
    ...
    <div class="loader-overlay" id="loader">
        <div class="loader-circle">
            <div class="loader-image">
                <img src="https://raw.githubusercontent.com/waseempathan501/Photo-link-/refs/heads/main/IMG-20260625-WA0026.jpg" alt="MR WASEEM HACKER CLOUD Loading">
            </div>
        </div>
        <div class="loader-text">INITIALIZING MR WASEEM CLOUD</div>
    </div>
    ...
    <div class="container">
        <!-- Owner Brand Avatar (outer ring spins, photo inside stays still) -->
        <div class="top-avatar-wrap">
            <div class="top-avatar-ring">
        ...
        <header class="header">
            <div>
                <a href="#" class="brand">
                    ...
                    <div class="brand-text">
                        <h1>MR WASEEM HACKER HOSTING</h1>
                        <span>Ignite Your Projects</span>
                    </div>
                </a>
            </div>
            ...
            <div class="social-links">
                ...
                <a href="https://tiktok.com/@waseempathan902" target="_blank" class="social-btn Tiktok" title="Tiktok Account">
        ...
        <div class="user-bar">
            <div class="user-bar-avatar">
                <img src="<?php echo htmlspecialchars(OWNER_PHOTO_URL); ?>" alt="">
            </div>
            <div class="user-bar-text">
                <span class="user-bar-label">Logged in as</span>
                <span class="user-bar-name"><?php echo htmlspecialchars($current_username); ?></span>
            </div>
            <a href="logout.php" class="user-bar-logout"><i class="fas fa-sign-out-alt"></i> Logout</a>
        </div>

        <!-- Hero -->
        <div class="hero">
            <h2>Your Code, Live In Seconds</h2>
            <p>Upload files, paste code, or connect a GitHub repo — push updates and watch them go live automatically, no redeploying needed.</p>
        ...
        <div class="dashboard">
            ...
            <div class="card">
                ...
                <div class="tabs">
                    ...
                    <div class="tab" onclick="switchTab('tab-github')"><i class="fab fa-github"></i> GitHub</div>
                </div>
...
<body>
    <!-- Your code here -->
                ...
                <div id="tab-github" class="tab-content">
                    <?php if (!$github_connected): ?>
                        <div style="text-align:center; padding: 30px 10px;">
                            <i class="fab fa-github" style="font-size: 3rem; color: var(--text-primary); margin-bottom: 15px;"></i>
                            <div style="font-weight:700; font-size:1.05rem; margin-bottom:8px;">Connect Your GitHub Account</div>
                            <div style="color: var(--text-muted); font-size:0.85rem; margin-bottom: 20px; line-height:1.5;">
                                Deploy any repo straight from GitHub. Once connected, every push to that repo updates your live site automatically — no manual redeploy needed.
                            </div>
                            <a href="github_auth.php" class="btn btn-primary" style="text-decoration:none; display:inline-flex;">
                                <i class="fab fa-github"></i> CONNECT GITHUB
                            </a>
                        </div>
                    <?php else: ?>
                        <div style="display:flex; align-items:center; justify-content:space-between; background: var(--bg-tertiary); border:1px solid var(--border-color); border-radius: var(--radius-md); padding: 10px 14px; margin-bottom: 16px;">
                            <div style="font-size:0.85rem;">
                                <i class="fab fa-github"></i> Connected as <strong><?php echo htmlspecialchars($github_username); ?></strong>
                            </div>
                            <a href="github_auth.php?disconnect=1" style="color: var(--text-muted); font-size:0.8rem; text-decoration:none;">Disconnect</a>
                        </div>

                        <form method="POST" action="github_deploy.php" id="github-deploy-form">
                            <div class="form-group">
                                <label>Select Repository</label>
                                <select name="repo_full_name" id="gh-repo-select" required style="width:100%; padding:12px 14px; background: var(--bg-tertiary); border:1px solid var(--border-color); border-radius: var(--radius-md); color: var(--text-primary); font-family: inherit;">
                            ...
                            <div class="form-group">
                                <label>Branch</label>
                                <input type="text" name="branch" id="gh-branch" placeholder="main" value="main">
                            </div>

                            <div class="form-group">
                                <label>Project Name</label>
                                <input type="text" name="custom_url" id="inp-gh-url" placeholder="my-github-site" required>
                                <div class="url-box">
                                    .../view.php?site=<span id="prev-gh-url">my-github-site</span>
                                </div>
                            </div>
                            ...
                            <div style="color: var(--text-muted); font-size:0.78rem; margin-top:10px; line-height:1.5;">
                                <i class="fas fa-sync"></i> A webhook will be added to this repo automatically — future pushes redeploy this site by themselves.
                            </div>
                        </form>
                    <?php endif; ?>
                </div>
                ...
                <div class="site-list">
                    ...
                    <?php else: ?>
                        <?php foreach($uploaded_sites as $site): ?>
                            <div class="site-item">
                                <?php 
                                    $iconClass = 'icon-zip'; $faIcon = 'fa-file-archive'; $faStyle = 'fab';
                                    ...
                                    elseif($site['type']=='github') { $iconClass = 'icon-zip'; $faIcon = 'fa-github'; }
                                ?>
                                <div class="file-icon <?php echo $iconClass; ?>">
                                    <i class="<?php echo $faStyle; ?> <?php echo $faIcon; ?>"></i>
                                </div>
                                <div class="site-info">
                                    <span class="site-name">
                                        <?php echo $site['name']; ?>
                                        <?php if($site['type']=='github'): ?>
                                            <span style="font-size:0.7rem; color: var(--text-muted); font-weight:500;">
                                                <i class="fas fa-sync" style="font-size:0.65rem;"></i> synced with <?php echo htmlspecialchars($site['repo']); ?>
                                            </span>
                                        <?php endif; ?>
                                    </span>
        ...
        <footer>
            <p>&copy; <?php echo date('Y'); ?> <strong class="text-gradient">MR WASEEM HOSTING</strong>. POWERED BY WASEEM HACKER.</p>
    ...
    <script>
        ...
        <?php if ($github_connected): ?>
        setupUrlBinding('inp-gh-url', 'prev-gh-url');

        // Load the user's GitHub repos into the dropdown
        fetch('github_repos.php')
            ...
            .then(data => {
                ...
                if (data.success && data.repos.length) {
                    select.innerHTML = '<option value="">Choose a repository...</option>' +
                        data.repos.map(r => `<option value="${r.full_name}" data-branch="${r.default_branch}">${r.full_name}${r.private ? ' 🔒' : ''}</option>`).join('');
                    ...
                    select.addEventListener('change', function() {
                        ...
                        const slug = this.value.split('/').pop()
                            .toLowerCase().replace(/[^a-z0-9]/g, '-').replace(/-+/g, '-').replace(/^-|-$/g, '');
                        const urlInput = document.getElementById('inp-gh-url');
                        const urlPreview = document.getElementById('prev-gh-url');
            ...
            .catch(() => {
                const select = document.getElementById('gh-repo-select');
                if (select) select.innerHTML = '<option value="">Failed to load repos</option>';
            });
        <?php endif; ?>
