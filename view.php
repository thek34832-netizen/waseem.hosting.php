...
// Get requested site name + owning user
$site_name = isset($_GET['site']) ? trim($_GET['site']) : '';
$user_name = isset($_GET['user']) ? trim($_GET['user']) : '';

if (empty($site_name) || empty($user_name)) {
...
if (
    strpos($site_name, '..') !== false || strpos($site_name, '/') !== false || strpos($site_name, '\\') !== false ||
    strpos($user_name, '..') !== false || strpos($user_name, '/') !== false || strpos($user_name, '\\') !== false
) {
...
$user_dir = SITES_DIR . '/' . $user_name;

// First check if it's a directory (ZIP extracted / GitHub deployed site)
$dir_path = $user_dir . '/' . $site_name;
if (is_dir($dir_path)) {
    // Look for HTML files in the directory
    $html_files = glob($dir_path . '/*.{html,htm}', GLOB_BRACE);
    if (!empty($html_files)) {
        $file_path = pickMainHtmlFile($html_files); // index.html wins, else first file
        $is_zip_site = true;
    }
} else {
    // Look for single files with different extensions
    foreach ($extensions as $ext) {
        $possible_path = $user_dir . '/' . $site_name . '.' . $ext;
