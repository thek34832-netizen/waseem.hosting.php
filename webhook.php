...
$deploy_key = ($deployment['username'] ?? '') . '/' . ($deployment['custom_url'] ?? '');
$target_dir = userSitesDir($deployment['username']) . '/' . $deployment['custom_url'];
...
if ($result['success']) {
    ...
    if (isset($deployments[$deploy_key])) {
        $deployments[$deploy_key]['updated_at'] = date('c');
