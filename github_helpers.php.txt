...
function ghFindDeploymentByRepo($repo_full_name) {
    foreach (ghGetDeployments() as $custom_url => $info) {
...
/**
 * Download a repo's zipball and extract it into sites/{custom_url}/
 ...
 */
function ghDeployRepoToSite($owner, $repo, $branch, $custom_url, $token = null) {
    if (!class_exists('ZipArchive')) {
        return ['success' => false, 'message' => 'ZIP extension missing on server.'];
    }
    ...
    $target_dir = $sites_dir . '/' . $custom_url;
